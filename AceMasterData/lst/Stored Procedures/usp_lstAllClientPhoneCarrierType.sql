...
CREATE PROCEDURE	[lst].[usp_lstAllClientPhoneCarrierType]
...
--Create Triggers 
	...
	'[LoadDate] Date NULL,'+
	'[DataDate] Date NULL,'+
	'[lstPhoneCarrierTypeKey] [int] PRIMARY KEY IDENTITY(1,1) NOT NULL,	'+
...
SET  @SqlString = 
		...
		, [SrcFileName], [LoadDate], [DataDate], [lstPhoneCarrierTypeKey]
		, [PhoneCarrierTypeName], [PhoneCarrierTypeCode], [ACTIVE], [EffectiveDate]
		, [ExpirationDate]'   + ')' +

		'SELECT		[CreatedDate], [CreatedBy], [LastUpdated], [LastUpdatedBy]
		, [SrcFileName], [LoadDate], [DataDate], [lstPhoneCarrierTypeKey]
		, [PhoneCarrierTypeName], [PhoneCarrierTypeCode], [ACTIVE]
		, [EffectiveDate], [ExpirationDate]'
				+
		' FROM		[AceMasterData].[lst].[lstPhoneCarrierType]'
				+
...
END
	...
	--Master -DONT TOUCH
	--SELECT * FROM [lst].[lstPhoneCarrierType]
	--Targets
	/*
	SELECT * FROM ACDW_CLMS_SHCN_MSSP.lst.lstPhoneCarrierType
	SELECT * FROM ACDW_CLMS_DHTX.lst.lstPhoneCarrierType
	*/
