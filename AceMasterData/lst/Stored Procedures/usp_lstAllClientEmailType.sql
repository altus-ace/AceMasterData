...
CREATE PROCEDURE		[lst].[usp_lstAllClientEmailType]
...
--Create Triggers 
	...
	'[LoadDate] Date NULL,'+
	'[DataDate] Date NULL,'+
	'[lstEmailTypeKey] [int] PRIMARY KEY IDENTITY(1,1) NOT NULL,	'+
...
SET  @SqlString = 
		...
		, [SrcFileName], [LoadDate], [DataDate], [lstEmailTypeKey]
		, [EmailTypeName], [EmailTypeCode], [ACTIVE], [EffectiveDate]
		, [ExpirationDate]'   + ')' +

		'SELECT		[CreatedDate], [CreatedBy], [LastUpdated], [LastUpdatedBy]
		, [SrcFileName], [LoadDate], [DataDate], [lstEmailTypeKey]
		, [EmailTypeName], [EmailTypeCode], [ACTIVE], [EffectiveDate], [ExpirationDate]'
				+
		' FROM		[AceMasterData].[lst].[lstEmailType]'
				+
...
END
	...
	--Master -DONT TOUCH
	--SELECT * FROM [lst].[lstEmailType]
	--Targets
	/*
	SELECT * FROM ACDW_CLMS_SHCN_MSSP.lst.lstEmailType
	SELECT * FROM ACDW_CLMS_DHTX.lst.lstEmailType
	*/
