...
CREATE PROCEDURE	[lst].[usp_lstAllClientPhoneType]
...
--Create Triggers 
	...
	'[LoadDate] Date NULL,'+
	'[DataDate] Date NULL,'+
	'[lstPhoneTypeKey] [int] PRIMARY KEY IDENTITY(1,1) NOT NULL,	'+
...
SET  @SqlString = 
		...
		, [LoadDate], [DataDate]
		, [lstPhoneTypeKey], [PhoneTypeName], [PhoneTypeCode], [ACTIVE]
		, [EffectiveDate], [ExpirationDate]'   + ')' +

		'SELECT		[CreatedDate], [CreatedBy], [LastUpdated], [LastUpdatedBy]
		, [SrcFileName], [LoadDate], [DataDate],[lstPhoneTypeKey], [PhoneTypeName], [PhoneTypeCode]
		, [ACTIVE], [EffectiveDate], [ExpirationDate]'
				+
		' FROM		[AceMasterData].[lst].[lstPhoneType]'
				+
...
END
	...
	--Master -DONT TOUCH
	--SELECT * FROM [lst].[lstPhoneType]
	--Targets
	/*
	SELECT * FROM ACDW_CLMS_SHCN_MSSP.lst.lstPhoneType
	SELECT * FROM ACDW_CLMS_DHTX.lst.lstPhoneType
	SELECT * FROM 
	*/
	
	/** USAGE
	EXECUTE [lst].[usp_lstAllClientPhoneType]'ACDW_CLMS_SHCN_MSSP.lst.lstPhoneType'
	**/
